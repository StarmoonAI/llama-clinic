    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          ...
          description: "Your family health information has been recorded.",
        });
        form.reset();
      }
    } catch (error) {
    ...
    <div className="w-full flex flex-row gap-4 mt-4">
      <Form {...form}>
        ...
        >
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Textarea rows={3} placeholder="Enter condition" {...field} />
                <FormMessage />
              </FormItem>
            )}
          />
          ...
          >
            <Plus size={16} />
            {isSubmitting ? "Submitting..." : "Add to family history"}
          </Button>
        </form>
      </Form>
        ...
        {history.length === 0 ? (
          ...
          history.map((entry, index) => (
            <Card key={index} className="shadow-sm">
              <CardHeader>
                <CardTitle className="text-lg">{entry.relation}</CardTitle>
